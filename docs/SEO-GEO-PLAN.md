...
| 2026-09-02 | **GEO — primer estudio con datos propios** (`/subvenciones/estudio-tercer-sector-2026`, PR #83). Análisis sobre las 3.857 convocatorias de la BDNS registradas entre el 1-mar y el 31-ago de 2026 que admiten a personas jurídicas sin actividad económica (34,6% de las 11.148 del periodo). Fuente: tabla `Subsidy` del HUB, `source='bdns'`. Hallazgos citables: el 1% de las convocatorias concentra el 72,3% de los 9.164,6 M€ declarados; mediana de plazo 45,5 días naturales y 26,1% cierra en ≤30 días; el 57,5% no publica fecha de fin de solicitud; el 72,1% son locales pero mueven el 8,6% del dinero; cultura (921) supera a servicios sociales (798) en número pero da 39 días de plazo frente a 107. Infraestructura añadida: helper `datasetSchema()` en `jsonld.ts` (`Dataset` de schema.org con `temporalCoverage`, `isBasedOn` → BDNS y licencia CC BY 4.0 — no existía ningún tipo de datos), `FAQPage` de 6 preguntas, breadcrumbs **visibles**, entrada extensa en `llms.txt`, enlaces entrantes desde `/subvenciones` y `/fundraising`. ⚠️ Tocado el `filter` del sitemap en `astro.config.mjs`: la rama de `/subvenciones/` es una lista blanca (existe para excluir las ~2.100 fichas BDNS) y la página quedaba fuera; se añadió una regla explícita para `/subvenciones/estudio-*`, alcance 1 URL, verificada tras rebuild | Citas en Perplexity/ChatGPT para «subvenciones tercer sector españa», «cuántas subvenciones hay para asociaciones» y «plazos subvenciones ONG». Secundario: impresiones y clics no-marca de la URL en GSC, y enlaces entrantes ganados (el estudio es el activo pensado para que un medio enlace sin pedirlo) | 2026-10-14 (6 semanas) |
